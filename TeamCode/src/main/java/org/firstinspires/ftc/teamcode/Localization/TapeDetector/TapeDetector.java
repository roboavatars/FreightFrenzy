...
public class TapeDetector {
    ...
    public double[] update(double x, double y, double theta) {
        ...
        if (!resetOdo && !right.isNull && !left.isNull && Math.abs(left.theta - right.theta) < TAPE_THETA_THRESHOLD) {
            ...
            if (entering) {
                updatedCoords[0] = 96 - TAPE_SENSOR_CENTER_TO_ROBOT_CENTER * sin(updatedCoords[1]);
            } else {
                updatedCoords[0] = 98 - TAPE_SENSOR_CENTER_TO_ROBOT_CENTER * sin(updatedCoords[1]);
            }
            resetOdo = true;
